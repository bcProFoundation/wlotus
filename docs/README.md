# Docs map

Start at **[STATUS.md](./STATUS.md)** (live token ids + 102/6). Confirm `/api/status` if git JSON disagrees.

| Doc | Read for |
|-----|----------|
| [STATUS.md](./STATUS.md) | What’s live |
| [VISION.md](./VISION.md) | Why W Lotus exists |
| [SPEC.md](./SPEC.md) | Consensus knobs |
| [ECONOMICS.md](./ECONOMICS.md) | WLotusCovenant, 1:1 forks, 102/6, GLOTUS |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Genesis → remint → burn |
| [CLOCK.md](./CLOCK.md) | Moore calendar, sunset |
| [ALTAR.md](./ALTAR.md) | On-chain memorial schema |
| [TEMPLE_SPECIALS.md](./TEMPLE_SPECIALS.md) | Vu Lan / Cô Hồn / events |
| [MOBILE.md](./MOBILE.md) | PWA + TWA + Capacitor |

Deploy: [test](../deploy/contabo/README.md) · [prod](../deploy/contabo/PROD.md) · [102/6 cutover (done)](../deploy/contabo/PROD_CUTOVER_102_6.md)

Historical: [PROPOSAL.md](./PROPOSAL.md) (ALP on eCash, not an L1) · [research/](./research/README.md). [ECONOMICS_WLOTUS_GLOTUS.md](./ECONOMICS_WLOTUS_GLOTUS.md) redirects here.
