...
describe('resolveMooreDaysPerExtraBit', () => {
  it('defaults to 500 (五百罗汉)', () => {
    expect(MOORE_DAYS_PER_EXTRA_BIT).toBe(500);
    expect(resolveMooreDaysPerExtraBit(undefined)).toBe(500);
    expect(resolveMooreDaysPerExtraBit('')).toBe(500);
  });

  it('clamps to 365–730 (~1–2 years)', () => {
    expect(resolveMooreDaysPerExtraBit('500')).toBe(500);
  ...
  it('maps to seconds for covenant bake', () => {
    expect(resolveProdSecondsPerExtraBit('500')).toBe(500 * 86_400);
