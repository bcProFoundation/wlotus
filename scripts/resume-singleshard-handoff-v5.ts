#!/usr/bin/env tsx
/**
 * Resume the single-shard v5 handoff after an interrupted
 * `create-singleshard-token-v5` run. Reads the PARTIAL dep record, funds
 * the shard if it still lacks its baton, completes the record. Idempotent.
 */
...
async function main(): Promise<void> {
  ...
  if (
    ...
    !dep.mode.startsWith('single-shard-delta-v5') ||
    !dep.tokenId
  ) {
    throw new Error('Live dep record is not a v5 single-shard partial (aborting)');
  }
