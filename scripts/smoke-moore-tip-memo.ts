...
async function main() {
  ...
  try {
    const code = readFileSync(
      'contracts/WlotusPowRemintMooreTipMemo.spedn',
      'utf8',
    );
    ...
    const mintAtomsLe = Buffer.alloc(6);
    mintAtomsLe.writeUInt32LE(1, 0);
    const dummy32 = Buffer.alloc(32, 0);
    const inst = new Ctor({
      tokenIdRev: Buffer.from(fromHexRev(tokenId)),
      mintAtomsLe,
