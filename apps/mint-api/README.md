...
- `MINT_SERVING_TIP_INDEX` (default **0**) — baton this desk spends (`0`…`27`). Test on the live token: `27` (last of 28 batons) so it does not race prod's tip 0. `MINT_SERVING_TIP_OFFSET` is a deprecated alias. mint-api loads `/etc/wlotus/mint.env` *before* `offer.ts` so this value is not stuck at 0.
